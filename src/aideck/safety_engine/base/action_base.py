...
from ..llm.aideck_llm import AideckLLMProvider
...
class ActionBase(ABC):
    ...
    def _detect_content_language(self, content: List[str]) -> str:
        ...
        if isinstance(self.language_identify_llm, AideckLLMProvider):
            llm = self.language_identify_llm
        else:
            llm = AideckLLMProvider(agent_name="Language Detection Agent", model=self.language_identify_llm)
        try:
    ...
    async def _detect_content_language_async(self, content: List[str]) -> str:
        ...
        if len(combined_text.strip()) == 0:
            return "en"
        # Use existing AideckLLMProvider if language_identify_llm is already one, otherwise create new
        if isinstance(self.language_identify_llm, AideckLLMProvider):
            llm = self.language_identify_llm
        else:
            llm = AideckLLMProvider(agent_name="Language Detection Agent", model=self.language_identify_llm)
    ...
    def _translate(self, text: str, target_language: str) -> str:
        ...
        if self.__class__.language != target_language:
            # Use existing AideckLLMProvider if base_llm is already one, otherwise create new
            if isinstance(self.base_llm, AideckLLMProvider):
                llm = self.base_llm
            else:
                llm = AideckLLMProvider(agent_name="Translation Agent", model=self.base_llm)
    ...
    def llm_raise_block_error(self, reason: str) -> PolicyOutput:
        ...
        if isinstance(self.base_llm, AideckLLMProvider):
            ...
            llm = AideckLLMProvider(agent_name="Block Error Message Agent", model=self.base_llm)
    ...
    def llm_raise_exception(self, reason: str) -> PolicyOutput:
        ...
        if isinstance(self.base_llm, AideckLLMProvider):
            ...
            llm = AideckLLMProvider(agent_name="Exception Message Agent", model=self.base_llm)
