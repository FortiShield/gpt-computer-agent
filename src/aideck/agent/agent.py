...
from aideck.canvas.canvas import Canvas

from aideck.utils.printing import print_price_id_summary, cache_hit, cache_miss, cache_stored, cache_configuration, agent_started
from aideck.cache import CacheManager
from aideck.agent.base import BaseAgent
from aideck.tools.processor import ToolProcessor, ExternalExecutionPause
from aideck.storage.base import Storage
from aideck.utils.retry import retryable
from aideck.utils.validators import validate_attachments_for_model
from aideck.storage.memory.memory import Memory
from aideck.models.base import BaseModelProvider
from aideck.models.factory import ModelFactory
from aideck.utils.package.exception import GuardrailValidationError
from aideck.safety_engine.base import Policy
from aideck.safety_engine.models import PolicyInput
from aideck.safety_engine.exceptions import DisallowedOperation
from aideck.utils.printing import policy_triggered

from aideck.agent.context_managers import (
    CallManager,
    ContextManager,
    ReliabilityManager,
    MemoryManager,
...
if TYPE_CHECKING:
    from aideck.tasks.tasks import Task

RetryMode = Literal["raise", "return_false"]

class Direct(BaseAgent):
    """Static methods for making direct LLM calls using the Aideck."""
    ...
    def __init__(self, 
        ...
        # Setup LLM models for AideckLLMProvider agents if policies use them
    ...
    def _setup_policy_llm_models(self, user_policy, agent_policy):
        """Setup LLM models for agents in AideckLLMProvider objects used by policies"""
        from aideck.safety_engine.llm.aideck_llm import AideckLLMProvider
        ...
        for policy in policies:
            ...
            if policy.base_llm is None and self.model_provider is not None:
                policy.base_llm = AideckLLMProvider(
                    agent_name="Policy Base Agent",
                    model=None  # Will be set below
                )
                policy.base_llm.agent.model_provider = self.model_provider
            
            # Check if policy.base_llm is an AideckLLMProvider
            elif isinstance(policy.base_llm, AideckLLMProvider):
                # Set the model for the AideckLLMProvider's agent if we have a model_provider
    ...
    async def agent_create(self, provider: BaseModelProvider, single_task: "Task", system_prompt: str):
        ...
        if not hasattr(self, '_aideck_wrapped_tools'):
            self._aideck_wrapped_tools = {}
        if not hasattr(agent_for_this_run, '_aideck_wrapped_tools'):
            agent_for_this_run._aideck_wrapped_tools = {}
        ...
        agent_for_this_run._aideck_wrapped_tools = self._aideck_wrapped_tools
    ...
    async def _apply_user_policy_async(self, task: "Task") -> (Optional["Task"], bool):
        ...
        try:
            ...
            from aideck.safety_engine.models import RuleOutput
