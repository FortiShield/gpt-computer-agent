...
def settings_popup(self):
    ...
    try:
        ...
        predefined_agents_button.setText("Install aideck[agentic]")

    online_tools_button = QPushButton(
        "Enable Aideck Tiger Tools - More Capability (Recommended)"
    )
    ...
    if is_online_tools_setting_active():
        online_tools_button.setText(
            "Disable Aideck Tiger Tools - Low Capability (Not Recommended)"
        )

        def deactivate_online_tools_():
            deactivate_online_tools_setting()
            the_main_window.update_from_thread(
                "Disabled Aideck Tiger Tools - Low Capability (Not Recommended)"
            )
            settings_dialog.close()
        ...
        def activate_online_tools_():
            ...
            the_main_window.update_from_thread(
                "Enabled Aideck Tiger Tools - More Capability (Recommended)"
            )
            settings_dialog.close()
    ...
    def save_api_key_(api_key):
        ...
        the_main_window.update_from_thread(
            "Wake word activated, just say 'Her Computer' or jarvis to activate the agent"
        )
        if first_time:
    ...
    if api_key == "CHANGE_ME":
        ...
        wake_word_button.setText("Please Install aideck[wakeword]")
    else:
