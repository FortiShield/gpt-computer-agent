"""
Aideck custom exceptions for better error handling and user experience.
"""
...
class AideckError(Exception):
    """Base exception for all Aideck-related errors."""
    pass


class APIKeyMissingError(AideckError):
    """
...
class ConfigurationError(AideckError):
    """
    ...
    def _display_error_panel(self):
        ...
        panel = Panel(
            table,
            title=f"[bold red]Aideck - {escape(self.error_type)}[/bold red]",
            border_style="red",
            expand=True,
            width=70
        )
...
class ProviderError(AideckError):
...
class RateLimitError(AideckError):
    """Exception raised when API rate limits are exceeded."""
    pass


class AuthenticationError(AideckError):
